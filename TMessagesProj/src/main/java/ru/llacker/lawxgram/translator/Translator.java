package ru.llacker.lawxgram.translator;
...
public class Translator {
    ...
    public static void showTranslateDialog(Context context, String query, ArrayList<TLRPC.MessageEntity> entities, boolean noforwards, BaseFragment fragment, Utilities.CallbackReturn<URLSpan, Boolean> onLinkPress, String sourceLanguage, View anchorView, Theme.ResourcesProvider resourcesProvider) {
        if (LawxConfig.transType == LawxConfig.TRANS_TYPE_EXTERNAL) {
            ...
            TranslateAlert2.showAlert(context, fragment, UserConfig.selectedAccount, sourceLanguage, LawxConfig.translationTarget, query, entities, noforwards, onLinkPress, null, resourcesProvider);
        }
    }
    ...
    public static ArrayList<String> getRestrictedLanguages() {
        ...
        if (LawxConfig.restrictedLanguages == null) {
            languages.add(stripLanguageCode(getCurrentTargetLanguage()));
        } else {
            languages.addAll(LawxConfig.restrictedLanguages);
        }
        return languages;
    }
    ...
    public static void saveRestrictedLanguages(List<String> restrictedLanguages) {
        ...
        if (!restrictedLanguages.isEmpty() && languages.isEmpty()) {
            LawxConfig.saveRestrictedLanguages(null);
            return;
        }
        LawxConfig.saveRestrictedLanguages(new HashSet<>(restrictedLanguages));
    }
    ...
    public static void showTranslationTargetSelector(BaseFragment fragment, View view, Runnable callback, Theme.ResourcesProvider resourcesProvider) {
        if (getCurrentTargetLanguages().size() <= 30) {
            ...
            PopupHelper.show(names, LocaleController.getString(R.string.TranslationTarget), targetLanguages.indexOf(LawxConfig.translationTarget), fragment.getParentActivity(), view, i -> {
                LawxConfig.setTranslationTarget(targetLanguages.get(i));
                if (callback != null) callback.run();
            }, resourcesProvider);
        } else {
            fragment.presentFragment(new LawxLanguagesSelectActivity(LawxLanguagesSelectActivity.TYPE_TARGET));
        }
    }
    ...
    public static void showTranslatorTypeSelector(Context context, View view, Runnable callback, Theme.ResourcesProvider resourcesProvider) {
        ...
        arrayList.add(LocaleController.getString(R.string.TranslatorTypeLawx));
        types.add(LawxConfig.TRANS_TYPE_LAWX);
        arrayList.add(LocaleController.getString(R.string.TranslatorTypeTG));
        types.add(LawxConfig.TRANS_TYPE_TG);
        arrayList.add(LocaleController.getString(R.string.TranslatorTypeExternal));
        types.add(LawxConfig.TRANS_TYPE_EXTERNAL);
        PopupHelper.show(arrayList, LocaleController.getString(R.string.TranslatorType), types.indexOf(LawxConfig.transType), context, view, i -> {
            LawxConfig.setTransType(types.get(i));
            if (callback != null) callback.run();
        }, resourcesProvider);
    }
    ...
    public static void showTranslationProviderSelector(Context context, View view, MessagesStorage.BooleanCallback callback, Theme.ResourcesProvider resourcesProvider) {
        ...
        PopupHelper.show(names, LocaleController.getString(R.string.TranslationProvider), types.indexOf(LawxConfig.translationProvider), context, view, i -> {
            var translator = getTranslator(types.get(i));
            String targetLanguage = getTargetLanguage(translator, LawxConfig.translationTarget);

            if (translator.supportLanguage(targetLanguage)) {
                LawxConfig.setTranslationProvider(types.get(i));
                if (callback != null) callback.run(true);
            } else {
                AlertDialog.Builder builder = new AlertDialog.Builder(context, resourcesProvider)
                        .setMessage(LocaleController.getString(R.string.TranslateApiUnsupported));
                if ("app".equals(LawxConfig.translationTarget)) {
                    builder.setPositiveButton(LocaleController.getString(R.string.UseGoogleTranslate), (dialog, which) -> {
                        LawxConfig.setTranslationProvider(PROVIDER_GOOGLE);
                        if (callback != null) callback.run(true);
                    });
                    builder.setNegativeButton(LocaleController.getString(R.string.Cancel), null);
    ...
    private static ITranslator getCurrentTranslator() {
        return getTranslator(LawxConfig.translationProvider);
    }
    ...
    public static String getCurrentTargetLanguage() {
        return getTargetLanguage(getCurrentTranslator(), LawxConfig.translationTarget);
    }
    ...
    private record TranslateTask(ITranslator translator, TLRPC.TL_textWithEntities query,
        ...
        public TranslationResult call() throws Exception {
            var key = Pair.create(query.text, tl + "|" + LawxConfig.translationProvider);
