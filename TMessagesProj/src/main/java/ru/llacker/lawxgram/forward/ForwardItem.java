package ru.llacker.lawxgram.forward;
...
public class ForwardItem {
    ...
    private static final String PREFS_NAME = "lawxforward";
    private static final String LEGACY_PREFS_NAME = "nekoforward";
    ...
    public static int getLastForwardOption(boolean hasCaption) {
        ...
        if (LawxConfig.showNoQuoteForward && lastOption == ID_FORWARD_NOQUOTE) {
            return ID_FORWARD;
        }
        if (!hasCaption && lastOption == ID_FORWARD_NOCAPTION) {
            return LawxConfig.showNoQuoteForward ? ID_FORWARD : ID_FORWARD_NOQUOTE;
        }
        return lastOption;
    }
}
