...
//        TLRPC.StickerSet stickerSet = messages_stickerSet.set;
    ...
    public ArrayList<TLRPC.MessageEntity> getEntities(CharSequence[] message, boolean allowStrike) {
        ...
        final String mono = "`";
        final String pre = "```";
        while (!(LawxConfig.newMarkdownParser || EditTextBoldCursor.disableMarkdown) && (index = TextUtils.indexOf(message[0], !isPre ? mono : pre, lastIndex)) != -1) {
            if (start == -1) {
        ...
        if (!EditTextBoldCursor.disableMarkdown && LawxConfig.newMarkdownParser) EntitiesHelper.parseMarkdown(message, allowStrike);
