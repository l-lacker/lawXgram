...
public class MediaDataController extends BaseController {
    ...
    public ArrayList<TLRPC.Document> getRecentStickers(int type, boolean firstEmpty) {
        ...
        ArrayList<TLRPC.Document> result = new ArrayList<>(arrayList.subList(0, Math.min(arrayList.size(), LawxConfig.maxRecentStickers)));
        if (firstEmpty && !result.isEmpty() && !StickersAlert.DISABLE_STICKER_EDITOR && !LawxConfig.minimizedStickerCreator) {
...
//        TLRPC.StickerSet stickerSet = messages_stickerSet.set;
    ...
    public ArrayList<TLRPC.MessageEntity> getEntities(CharSequence[] message, boolean allowStrike, boolean parseMarkdown) {
        ...
        final String mono = "`";
        final String pre = "```";
        while (parseMarkdown && !(LawxConfig.newMarkdownParser || EditTextBoldCursor.disableMarkdown) && (index = TextUtils.indexOf(message[0], !isPre ? mono : pre, lastIndex)) != -1) {
            if (start == -1) {
        ...
        if (parseMarkdown && !EditTextBoldCursor.disableMarkdown && LawxConfig.newMarkdownParser) EntitiesHelper.parseMarkdown(message, allowStrike);
...
//                }
        ...
        if (LawxConfig.newMarkdownParser || EditTextBoldCursor.disableMarkdown) return entities;
        if (parseMarkdown) {
