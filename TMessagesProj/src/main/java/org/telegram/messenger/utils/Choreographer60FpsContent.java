...
public final class Choreographer60FpsContent implements Choreographer.FrameCallback {
    ...
    private boolean hasWork() {
        if (!mOneShot.isEmpty() || !mDrawablesToInvalidate.isEmpty() || !mDrawablesToInvalidate30fps.isEmpty() || !mViewsToInvalidate.isEmpty()) {
            return true;
        }
    ...
    private void removeGroupIfEmpty(int index) {
        if (index >= 0 && index < mGroups.size() && isGroupEmpty(mGroups.valueAt(index))) {
            mGroups.removeAt(index);
        }
    }

    private boolean isGroupEmpty(CallbackGroup group) {
        return group.callbacks.isEmpty() && group.runnableCallbacks.isEmpty();
    }
    ...
    private static void checkMainThread() {
        if (BuildConfig.DEBUG_PRIVATE_VERSION || BuildVars.DEBUG_VERSION) {
