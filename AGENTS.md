...
- Use a meaningful subject that describes the completed result, not a generic infinitive like `исправить`, `обновить`, or `добавить`. Good subjects read like `исправлена работа ...` or `добавлена поддержка ...`.
- Add a detailed commit body that plainly explains what was changed and why, with enough context to understand the exact work without opening the diff.
